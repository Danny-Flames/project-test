...
import { deCrypter, encrypter } from "../helpers/encrypt";
import { getUserFromLocalStorage } from "../helpers/localStorage";
  ...
  const setupInterceptors = (baseUrl?: string) => {
    ...
    apiWithoutToken.interceptors.request.use(
      (config: CustomRequestConfig) =>
        requestInterceptor(config, true, false, true), 
      (error) => Promise.reject(error)
    );
    ...
    // Set up response interceptors for apiWithToken and apiWithoutToken
